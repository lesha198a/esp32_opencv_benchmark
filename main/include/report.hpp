...
using functionSignature2 = std::function<int (const imagesList &)>;
...
public:
    ...
    virtual void executeOn(const imagesList &testImages);
};

// ==============
// class TestCase
// ==============
class TestCaseOnAll : public TestCase{
public:
    functionSignature2 funcOnAll;

    TestCaseOnAll(const std::string &_name, functionSignature2 _func);

    void executeOn(const imagesList &testImages) override;
};
...
public:
    ...
    void addTestCase(std::unique_ptr<TestCase> test);
};
