...
// TestCase class
TestCase::TestCase(string _name, int _iterations, function<int(const cv::Mat &)> _func) : name(move(_name)),
    iterations(_iterations), func(move(_func)) {
}

TestCase::TestCase(string _name, function<int(const cv::Mat &)> _func) : TestCase(
    move(_name), DEFAULT_ITERATIONS, move(_func)) {
}
...
TestCaseOnAll::TestCaseOnAll(const std::string &_name, functionSignature2 _func) : TestCase(_name, {}),
    funcOnAll(std::move(_func)) {
}

void TestCaseOnAll::executeOn(const imagesList &testImages) {
    float timeTaken;
    try {
        timeTaken = (float) funcOnAll(testImages) / 1000.0f;
    } catch (cv::Exception &e) {
        cout << "Exception while trying to execute function " << name << " :\n" << e.msg;
        timeTaken = -1;
    }
    resultTime.push_back(timeTaken);
    cout << "Testing " << name << " on  took "
            << resultTime.back() << " [ms] (in " << iterations << " iterations)" << endl;
}
...
void TestGroup::addTestCase(std::unique_ptr<TestCase> test) {
...
string Summary::lineText(const vector<string> &colTexts) {
    ...
    for (int c = 0; c < numOfTextCol; c++) {
        ...
        if ((c == numOfTextCol - 1) && (c < columnsWidth.size() - 1)) {
            ...
            line += (c == 0
                         ? alignTextLeft(colTexts[c], columnsWidth.at(c))
                         : alignTextRight(colTexts[c], columnsWidth.at(c)));
